#!/bin/sh
# Anseo SDK drift gate (Story 12.3 / NFR-OpenAPIStable).
#
...
if [ -d "$PY_DIR/anseo" ]; then
  cp -R "$PY_DIR/anseo/." "$SNAP_PY/"
fi
...
if command -v uvx >/dev/null 2>&1; then
  ...
  echo "::warning::neither uvx nor openapi-python-client installed; skipping Python drift check"
fi

# Cleanup runtime artifacts that aren't part of the SDK shape.
rm -rf "$PY_DIR/anseo/.ruff_cache" "$PY_DIR/anseo/__pycache__" || true
...
if [ "$PY_OK" -eq 1 ]; then
  if ! diff -q -r \
      ...
      --exclude=".ruff_cache" \
      --exclude=".gitignore" \
      "$PY_DIR/anseo" "$SNAP_PY" >/dev/null 2>&1; then
...
if [ "$DRIFT" -eq 0 ]; then
  rm -rf "$TS_DIR/src" "$PY_DIR/anseo"
  mkdir -p "$TS_DIR/src" "$PY_DIR/anseo"
  cp -R "$SNAP_TS/." "$TS_DIR/src/"
  cp -R "$SNAP_PY/." "$PY_DIR/anseo/"
