...
//!   `/v1` server logs. (The transport-level `X-OpenGEO-Request-Id` is the
//!   same value.)
...
//! `X-OpenGEO-Project: <project>` on every `/v1` call. The header value type
...
/// as the `X-OpenGEO-Project` header to `/v1` (Story 0.11 wires the header).
///
