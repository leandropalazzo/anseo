//! Mistral provider adapter — Phase 2 Story 11.2.
//!
//! Mistral exposes an OpenAI-compatible `POST /v1/chat/completions` endpoint
//! at `api.mistral.ai`. Same wire shape as OpenAI; the model allowlist is
//! the Mistral-specific bit.

use async_trait::async_trait;

use anseo_core::{ProviderName, Secret, DEFAULT_MISTRAL_MODEL};
...
impl MistralProvider {
    pub fn new(api_key: Secret) -> Self {
        Self::with_base_url(api_key, DEFAULT_MISTRAL_BASE_URL)
    }
...
impl Provider for MistralProvider {
    ...
    fn validate_model(&self, model: &str) -> Result<String, ProviderError> {
        if SUPPORTED_MODELS.contains(&model) {
            ...
            Err(ProviderError::unsupported_model(format!(
                "unsupported Mistral model `{model}` (supported: {})",
    ...
    async fn run(&self, request: ProviderRequest) -> Result<ProviderResponse, ProviderError> {
        self.http.validate_endpoint(&ProviderName::Mistral)?;
        ...
        let raw: serde_json::Value = serde_json::from_str(&raw_text).map_err(|e| {
            ProviderError::invalid_response(format!("non-JSON Mistral response: {e}"))
        })?;

        let message_text = extract_message_text(&raw).ok_or_else(|| {
            ProviderError::invalid_response("Mistral response missing choices[0].message.content")
        })?;
...
mod tests {
    ...
    fn validate_model_accepts_supported() {
        let p = MistralProvider::new(Secret::new("test"));
        assert!(p.validate_model("mistral-large-latest").is_ok());
    }

    #[test]
    fn validate_model_rejects_unknown_with_unsupported_model_kind() {
        let p = MistralProvider::new(Secret::new("test"));
        let err = p.validate_model("mistral-99").unwrap_err();
