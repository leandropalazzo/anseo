...
Story 40.5 closes the convenience loop around that published suite:

- `anseo suite list` prints the canonical slugs, one per line.
- `anseo suite check <slug>` exits `0` when the slug is canonical and `1` when it is not, which makes it CI-friendly.
- `GET /v1/suite/prompts` returns the same canonical list as JSON (`[{ slug, version, description }]`).
- MCP exposes the same list through `list_suite_prompts`.

Current `geo-v1` canonical slugs:

| Slug | Category | Description |
|---|---|---|
| `geo-v1/best-vector-db` | `platform-selection` | Category-comparison query for vector database selection. |
| `geo-v1/best-rag-platform` | `platform-selection` | Category-comparison query for full RAG platform evaluation. |
| `geo-v1/llm-observability-tools` | `observability` | Tooling-comparison query for LLM observability products. |
| `geo-v1/ai-search-visibility-platforms` | `brand-visibility` | Category query for AI-search visibility and citation monitoring platforms. |
| `geo-v1/best-enterprise-chatbot-platform` | `application-platform` | Category query for enterprise chatbot and assistant platforms. |
| `geo-v1/agent-frameworks` | `developer-frameworks` | Framework-comparison query for agent orchestration libraries. |
| `geo-v1/best-ai-evaluation-tools` | `evaluation` | Tooling-comparison query for LLM evaluation and regression testing products. |
| `geo-v1/customer-support-ai` | `use-case-solutions` | Solution-comparison query for customer-support AI products. |

The comparable-contribution recipe is:
